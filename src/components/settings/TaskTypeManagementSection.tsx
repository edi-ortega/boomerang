...
export default function TaskTypeManagementSection() {
  ...
  const loadTaskTypes = async () => {
    ...
    try {
      if (!currentTenantId) {
        console.warn("⚠️ [TaskType] No client_id found");
        setTaskTypes([]);
        setIsLoading(false);
        return;
      }
      
      const { data, error } = await select("prj_task_type", "*").order("order");
      
      if (error) throw error;
      
      console.log("📦 [TaskType] Loaded task types:", data?.length || 0, "items");
      
      setTaskTypes(data || []);
    } catch (error) {
      console.error("❌ [TaskType] Error loading task types:", error);
      toast({
        title: "Erro ao carregar",
        description: "Não foi possível carregar os tipos de tarefa.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    try {
      ...
      if (editingType) {
        const { data, error } = await (await import("@/lib/supabase-client")).getSupabaseClient()
          .then(client => client.rpc('update_task_type', {
            p_user_id: userId,
            p_id: editingType.id,
        ...
        const { data, error } = await (await import("@/lib/supabase-client")).getSupabaseClient()
          .then(client => client.rpc('insert_task_type', {
            p_user_id: userId,
            p_name: formData.name,
            p_code: formData.code,
            p_description: formData.description,
      ...
      toast({
        ...
        description: "Não foi possível salvar o tipo de tarefa.",
        variant: "destructive"
      });
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const { data, error } = await (await import("@/lib/supabase-client")).getSupabaseClient()
        .then(client => client.rpc('delete_task_type', {
          p_user_id: userId,
          p_id: id
        }));
      ...
      toast({
        title: "Tipo excluído!",
        description: "O tipo de tarefa foi excluído com sucesso."
      });
      await loadTaskTypes();
    } catch (error) {
      ...
      toast({
        ...
        description: "Não foi possível excluir o tipo de tarefa.",
        variant: "destructive"
      });
    }
  };
  ...
  return (
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Tipos de Tarefa</h2>
          <p className="text-muted-foreground">Gerencie os tipos de tarefas do sistema</p>
        </div>
          ...
          <Card className="glass-effect border-border">
            <CardHeader>
              <CardTitle className="text-foreground">
                {editingType ? "Editar Tipo de Tarefa" : "Novo Tipo de Tarefa"}
              </CardTitle>
            </CardHeader>
            <CardContent>
      ...
      {isLoading ? (
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <CheckSquare className="w-6 h-6" style={{ color: type.color }} />
                        </div>
                        <div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        <div 
                          ...
                          style={{ backgroundColor: `${type.color}20` }}
                        >
                          <CheckSquare className="w-5 h-5" style={{ color: type.color }} />
                        </div>
        ...
        <Card className="glass-effect border-border">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-foreground mb-2">Nenhum tipo de tarefa cadastrado</h3>
            <p className="text-muted-foreground mb-6">Crie seu primeiro tipo de tarefa</p>
